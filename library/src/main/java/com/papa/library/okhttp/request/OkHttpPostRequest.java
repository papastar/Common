...
public class OkHttpPostRequest extends OkHttpRequest {
    ...
    protected OkHttpPostRequest(String url, String tag, Map<String, String> params, Map<String,
            String> headers, String content, byte[] bytes, File file) {
    ...
    protected void validParams() {
        ...
        if (count <= 0 || count > 1) {
            throw new IllegalArgumentException("the params , content , file , bytes must has one " +
                    "and only one .");
        }
    }
